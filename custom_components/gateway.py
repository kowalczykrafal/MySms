...
from .const import DOMAIN, SMS_STATE_UNREAD, CONF_SMS_STATE, DEFAULT_SMS_STATE 
...
class Gateway:
    ...
    def sms_read_messages(self, state_machine, force=False):
        ...
        for entry in entries:
            ...
            sms_state_opt = entry.data.get(CONF_SMS_STATE, DEFAULT_SMS_STATE)
            _LOGGER.debug("SMS state:%s", sms_state)
            _LOGGER.debug("Status Message:%s", sms_state_opt)
            if sms_state == sms_state_opt or sms_state_opt == "All":
